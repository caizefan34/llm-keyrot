...
const RATE_CODES = new Set(['RATE_LIMIT', 'QUOTA'])
...
export class KeyRotator {
  ...
  /** Dispose the rotator: cancel all pending timers and reject active waits. */
  dispose() {
  ...
  async onRateLimit(provider, failure, signal) {
    ...
    return this._withLock(provider, async () => {
      ...
      if (!await cancellableDelay(waitMs, signal)) return undefined
